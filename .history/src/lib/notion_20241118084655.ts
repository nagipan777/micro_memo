...
const validateEnv = () => {
  try {
    ...
    console.error('環境変数の検証に失敗:', JSON.stringify(error, null, 2));
...
const getNotionClient = () => {
  ...
  // Notion クライアントの fetch オプションをカスタマイズ
  return new Client({
    auth: env.VITE_NOTION_KEY,
...
const notionToEntry = (page: any): DiaryEntry => {
  ...
  return {
    id: page.id,
    title: page.properties?.タイトル?.title?.[0]?.plain_text || 'Untitled',
    content: page.properties?.内容?.rich_text?.[0]?.plain_text || 'No content',
    date: new Date(page.properties?.日付?.date?.start || new Date()),
    mood: page.properties?.気分タグ?.multi_select?.[0]?.name || 'No mood',
    };
};
...
export async function fetchEntries(): Promise<DiaryEntry[]> {
  ...
  try {
    console.log('Fetching entries with:', {
    ...
    console.error('Failed to fetch entries:', {
      ...
      raw: JSON.stringify(error, null, 2), // 詳細なエラー内容を出力
    });
    throw new Error('Failed to fetch entries from Notion API');
  }
}
