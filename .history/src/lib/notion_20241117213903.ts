...
export async function fetchEntries(): Promise<DiaryEntry[]> {
  const notion = getNotionClient();
  const env = validateEnv();
  console.log('Database ID:', env.VITE_NOTION_DATABASE_ID); 
  try {
    console.log('Before API Call'); // 呼び出し直前のログ
    ...
    console.log('API Call Succeeded:', response); // 成功時のログ
    return response.results.map(notionToEntry);
  } catch (error) {
    console.error('API Call Failed:', error); // 失敗時のログ
  ...
  //   if (!response.results) {
  //     console.error('No results returned:', response);
  //     throw new Error('No results returned from Notion API');
  //   }

  //   return response.results.map(notionToEntry);
  // } catch (error) {
    console.error('Failed to fetch entries. Error Details:', {
